...
var wapply_deltas : Array = [] # buffer deltas. Deltas for updating weights / Deltas de búfer. Deltas para actualizar pesos
var bapply_deltas : Array = [] # buffer deltas. Deltas for updating biases / Deltas de búfer. Deltas para actualizar sesgos
var target : Array = []
var uf : Array = []
var f : Array = []
var df : Array = []
...
func train(input_data : Array, target_data : Array) -> void:
	...
	if input_data.size() != target_data.size():
		push_error(input_data.size(),": size input ",target_data.size()," :target size","Number of elements in input data array doesn't match number of elements in target data array")
		return
	match algorithm:
...
func set_input(input : Array) -> void:
	if input.size() != neurons_out[0].size():
		push_error("Input size must match number of neurons on the first layer input size: ", input.size()," size neuron out: ",neurons_out[0].size() )
		return
	neurons_out[0] = input.duplicate()

func set_target(new_target : Array) -> void:
	if target.size() != new_target.size():
		push_error("Target size must match number of neurons on the last layer , new_target: ",new_target.size(),"  target;size: ",target.size()  )
		return
